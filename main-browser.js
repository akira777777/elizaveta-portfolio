...
;(function () {
  ...
  // Защита от множественных выполнений скрипта
  if (window.__PortfolioAppInitialized) {
    console.warn('⚠️ PortfolioApp script already executed, skipping')
    return
  }
  window.__PortfolioAppInitialized = true
  ...
  class PortfolioApp {
    ...
    cacheElements() {
      ...
      this.cachedElements.scrollProgress =
        document.getElementById('scroll-progress')
      this.cachedElements.scrollProgressBar = document.getElementById(
        'scroll-progress-bar'
      )
    ...
    initLazyLoading() {
      ...
      if ('IntersectionObserver' in window) {
        ...
        const lazyImages = document.querySelectorAll(
          'img[loading="lazy"]:not([data-lazy-handled]), img[data-src]:not([data-lazy-handled])'
        )
    ...
    handleImageError(img) {
      // Предотвращаем бесконечные циклы - если это уже data URI или placeholder, не обрабатываем снова
      ...
      // Используем CSS для отображения placeholder вместо установки data URI как src
      // (data URI в src может вызывать проблемы в некоторых браузерах)
      img.style.backgroundColor = '#f0f0f0'
      img.style.backgroundImage = `url("data:image/svg+xml,${encodeURIComponent(`
      ...
      // Устанавливаем прозрачный 1x1 пиксель вместо пустой строки
      // (пустая строка вызывает загрузку base URL, что приводит к ошибкам)
      // Используем более надежный способ - удаляем srcset и устанавливаем безопасный data URI
      if (img.srcset) {
      ...
      // Удаляем обработчик ошибок, чтобы предотвратить повторные вызовы
      img.removeEventListener('error', img._errorHandler)
      delete img._errorHandler
    }
    ...
    setupScrollToTop() {
      ...
      const scrollBtn =
        this.cachedElements.backToTopBtn ||
  ...
  // Инициализация приложения (с защитой от множественных инициализаций)
  if (window.PortfolioApp && window.PortfolioApp.isInitialized) {
    console.warn(
      '⚠️ PortfolioApp already initialized, skipping duplicate initialization'
    )
    return
  }
